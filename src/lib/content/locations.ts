...
export const LOCATIONS: Location[] = [
  {
    ...
    intro: "From Qurum to Al Mouj, we help hotels, clinics, developers and retail groups build digital systems that compound for years. As search shifts from ranked results toward AI-generated answers and summaries, we build SEO for Muscat businesses around both — the foundations traditional search still rewards, and the clear, well-structured content AI search tools are understood to favour. Search behaviour here is genuinely bilingual — many customers search in Arabic, English or a mix of both — so local visibility and website content need to work in both, not read as an afterthought translation. With most local searches happening on a phone, a site that loads quickly and reads clearly on mobile tends to matter more here than in less mobile-first markets.",
